...
@baseline_registry.register_trainer(name="tent")
class TentTrainer(BaseRLTrainer):
    ...
    def __init__(self, config=None):
        ...
        apply_corruptions = self.config.robustness.apply_corruptions
        robustnav_corruption_types = ["Defocus Blur", "Motion Blur", "Lighting", "Spatter", "Speckle Noise"]
        albumentations_corrupton_types = ["Jitter", "Glare", "Light Out", "Shadow", "Snow", "Rain", "Fog", "Occlusion"]
        if apply_corruptions:
            if self.config.robustness.visual_corruption in robustnav_corruption_types:
                print(f"use robustnav: {self.config.robustness.visual_corruption}")
                self.my_benchmark = MyBenchmark(self.config)
            elif self.config.robustness.visual_corruption in albumentations_corrupton_types:
                print(f"use albumentations: {self.config.robustness.visual_corruption}")
                self.my_benchmark = AlbumentationsBenchmark(self.config)

        self.adaptation_episodes_infos = [] # For adaptation phase
        self.episode_count_dict = {} # For adaptation phase, to make sure that each episode has been seen only once
        self.adapt_done = False # For adaptation
        # print("USE TENT TRAINER (tent_trainer.py)")
    ...
    def _init_train(self, resume_state=None):
        ...
        # If adaptation use validation scenes
        with read_write(self.config):
            if self.config.adaptation.adaptation_phase:
        ...
        # for name, param in self._agent.actor_critic.visual_encoder.named_parameters():
        #     print(name)
        self._is_static_encoder = (
            not self.config.habitat_baselines.rl.ddppo.train_encoder
        )
        try:
            self._is_static_policy = (
                not self.config.adaptation.train_policy
            )
        except:
            self._is_static_policy = False
        self._ppo_cfg = self.config.habitat_baselines.rl.ppo
        self.num_episodes = sum(self.envs.number_of_episodes)
        observations = self.envs.reset()
        observations = self.envs.post_step(observations)
        if self.config.adaptation.adaptation_phase and self.config.robustness.apply_corruptions:
            for i in range(len(observations)):
                observations[i]["rgb"] = self.my_benchmark.corrupt_rgb_observation(observations[i]["rgb"])
                # observations[i]["rgb"] = observations[i]["rgb"] * 0 Sanity Check
        ...
        with g_timer.avg_time("trainer.update_stats"):
            ...
            if self.config.adaptation.adaptation_phase and self.config.robustness.apply_corruptions:
                for i in range(len(observations)):
                    observations[i]["rgb"] = self.my_benchmark.corrupt_rgb_observation(observations[i]["rgb"])
                    # observations[i]["rgb"] = observations[i]["rgb"] * 0 Sanity Check
            # Visualise the first 100 frames
            if self.count < 100:
                # print("self.count", self.count)
                if self.config.adaptation.adaptation_phase:
                    if self.config.robustness.apply_corruptions:
                        img_save_dir = f"../scratch/{self.config.adaptation.adaptation_method}/{self.config.robustness.visual_corruption}"
                    else:
                        img_save_dir = f"../scratch/{self.config.adaptation.adaptation_method}/clean"
                else:
                    img_save_dir = "../scratch/temp_clean/"
                if not(os.path.exists(img_save_dir)):
                    os.makedirs(img_save_dir)
                for i in range(len(observations)):
                    # cv2.imwrite(f'{self.config.habitat_baselines.video_dir}/{self.count}.png', cv2.cvtColor(observations[i]["rgb"], cv2.COLOR_RGB2BGR))
                    # print(f'saved at {img_save_dir}/{self.count}.png')
                    self.count+=1
            ...
            # dones = [done1, done2, ...]
            # This collect metrics of every training episode
            if self.config.adaptation.adaptation_phase:
                assert (len(dones) == 1), "Adaptation can be run using only one environment (PPO)"
                if dones[0]:
                    current_episodes_info = self.envs.current_episodes()
                    k = (
                        current_episodes_info[0].scene_id,
                    ...
                    episode_stats = {}
                    episode_stats.update(extracted_infos)

                    self.ep_eval_count[k] += 1
                    # use scene_id + episode_id as unique id for storing stats
                    self.stats_episodes[(k, self.ep_eval_count[k])] = episode_stats

                    for k, v in extracted_infos.items():
                        self.writer.add_scalar(f"adaptation_metrics/{k}", v[0], len(self.episode_count_dict.keys()))
                    if episode_id not in self.episode_count_dict.keys():
                        self.episode_count_dict[episode_id] = 1
                        self.adaptation_episodes_infos.append(extracted_infos)
                    else:
                        num_seen_episodes = len(self.episode_count_dict.keys())
                        print(f"Warning EPISODE {episode_id} is used for the second time")
                        print(f"The agent has seen distinct {num_seen_episodes} episodes")
                        self.adapt_done = True

                        # Copy from habitat_evaluator.py
                        aggregated_stats = {}
                        all_ks = set()
                        ...
                        for k, v in metrics.items():
                            self.writer.add_scalar(f"eval_adaptation_metrics/{k}", v, 0)
    ...
    def train(self) -> None:
        ...
        with (
            ...
            self.writer = writer
            while (not self.is_done()) and (not self.adapt_done):
                ...
                with g_timer.avg_time("trainer.rollout_collect"):
                    ...
                    for step in range(self._ppo_cfg.num_steps): # this loops over ppo steps (default is 128)
                        is_last_step = (
    ...
    def _eval_checkpoint(
        ...
        if self.config.habitat_baselines.eval.should_load_ckpt:
            ...
            try:
                ...
                step_id = 0
                logger.info("No step id is specified, number of training steps is unknown")
        ...
        try:
            ...
            with read_write(self.config):
                self.config.habitat.dataset.split = self.config.habitat_baselines.eval.split
        except:
            with read_write(self.config):
                self.config.habitat.dataset.split = self.config.habitat_baselines.eval.split
            config = self.config
        ...
        if (
            ...
            # print("ckpt_dict[state_dict]", ckpt_dict['state_dict'].keys())
            # model_weights = set()
            # for name, weight in self._agent.actor_critic.named_parameters():
            #     model_weights.add(name)
            #     if name not in ckpt_dict['state_dict'].keys():
            #         print(f"model weight {name} is not in the loaded weights")
            # for name in ckpt_dict['state_dict'].keys():
            #     if name not in model_weights:
            #         print(f"loaded weight {name} is not in the model weights")
        ...
        # Init adaptation if specified
        self._init_grad_free_adaptation()
    ...
    def _init_grad_free_adaptation(self):
        # For gradient-free adaptation like DUA
        try:
            self.adaptation_method = self.config.adaptation.adaptation_method.lower()
        except:
            self.adaptation_method = None
        self.mom_pre= None 
        self.decay_factor = None
        self.min_mom = None
        if self.config.adaptation.adaptation_phase and self.adaptation_method == "dua":
            self.mom_pre=0.1 # initial momentum of DUA, it will be not used for non adaptation
            self.decay_factor = self.config.adaptation.dua.decay_factor
            self.min_mom = self.config.adaptation.dua.min_mom
            print("Adaptation using DUA")
