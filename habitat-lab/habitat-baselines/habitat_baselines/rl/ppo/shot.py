...
from habitat_baselines.rl.ppo.policy import NetPolicyTent
...
class PPO(nn.Module, Updater):
    ...
    @classmethod
    def from_config(cls, actor_critic: NetPolicyTent, config):
        return cls(
    ...
    def _update_from_batch(self, batch, epoch, rollouts, learner_metrics):
        ...
        # L_ent = dist_entropy
        # L_div
        softmax_out = torch.exp(action_log_probs)
        msoftmax = softmax_out.mean(dim=0)
        L_div = torch.sum(msoftmax * torch.log(msoftmax + 1e-5))

        all_losses = []
        # Adjust Entropy Term of PPO loss (maxmise entropy) to be TENT (minimise entropy)

        # L_infomax = L_ent + L_div
        all_losses.append(dist_entropy + L_div)
