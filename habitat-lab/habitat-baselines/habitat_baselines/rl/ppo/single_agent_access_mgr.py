...
class SingleAgentAccessMgr(AgentAccessMgr):
    def __init__(
        ...
        try: # Adapt only encoder and freeze policy
            self._is_static_policy = (
                not config.adaptation.train_policy
            )
            self._is_adaptation = config.adaptation.adaptation_phase
        except: # Not all yaml files has adaptation.train_policy (the original does not have it )
            self._is_static_policy = False
            self._is_adaptation = False

        if self._is_adaptation:
            if "tent" in config.adaptation.adaptation_method:
                # Use TENT class that mimics PPO class
                from habitat_baselines.rl.ppo.tent import PPO
                print("use TENT class (single_agent_access_mgr.py)")
            elif "shot" in config.adaptation.adaptation_method:
                # Use SHOT class that mimics PPO class
                from habitat_baselines.rl.ppo.shot import PPO
                print("use SHOT class (single_agent_access_mgr.py)")                
    ...
    def _create_policy(self) -> NetPolicy:
        ...
        """
        print("Create policy name", self._config.habitat_baselines.rl.policy[self.agent_name].name)
        ...
        if self._config.habitat_baselines.rl.ddppo.pretrained:
            print(f"Load pretrained model from {self._config.habitat_baselines.rl.ddppo.pretrained_weights}")
            try:
                ...
                print("Failed to load the state dict strictly, set strict = False")
                actor_critic.load_state_dict(
                    {  # type: ignore
                        k[len("") :]: v
                        for k, v in pretrained_state["state_dict"].items()
            ...
            try:
                ...
                print("Failed to load state dict of visual encoder strictly, set strict = False")
                actor_critic.net.visual_encoder.load_state_dict(
                    {
                        k[len(prefix) :]: v
                        for k, v in pretrained_state["state_dict"].items()
        ...
        if self._is_adaptation and self._is_static_policy:
            for param in actor_critic.parameters():
                param.requires_grad_(False)
            for param in actor_critic.visual_encoder.parameters():
                param.requires_grad_(True) # Train only visual encoder
            print("Freeze Policy")

        if self._is_static_encoder and not(self._is_adaptation):
            for param in actor_critic.visual_encoder.parameters():
                param.requires_grad_(False)
        ...
        if self._is_adaptation and ("tent" in self._config.adaptation.adaptation_method or "shot" in self._config.adaptation.adaptation_method):
            for param in actor_critic.parameters():
                param.requires_grad_(False)

            for m in actor_critic.visual_encoder.modules():
                if isinstance(m, torch.nn.modules.batchnorm._BatchNorm):
                    m.requires_grad_(True)
                    m.train()
                    if self._config.adaptation.adaptation_method == "tent":
                        print("Deactivate Running Statistics")
        ...
        # SHOT: Adapt the whole encoder just like the original paper, is not stable
        # elif self._is_adaptation and "shot" in self._config.adaptation.adaptation_method:
        #     for param in actor_critic.parameters():
        #         param.requires_grad_(False)
        #     for m in actor_critic.visual_encoder.modules():
        #         m.requires_grad_(True)
